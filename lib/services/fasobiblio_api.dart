...
class FasobiblioApi {
  static const database =
      'https://alifam-hub-1b32a-default-rtdb.europe-west1.firebasedatabase.app';
  ...
  Future<UserSession> ensureSession() async {
    ...
    if (_session == null) {
      try {
        ...
        return UserSession.offlineGuest();
      }
    }
    if (_session!.expiresAt <= DateTime.now().millisecondsSinceEpoch) {
      try {
        return await _refresh(_session!);
      } catch (_) {
        return _guest();
      }
    }
  ...
  UserSession _fromAuth(
    ...
    expiresAt:
        ...
        ((num.tryParse('${data['expiresIn'] ?? 3600}') ?? 3600).toInt() *
            1000) -
        60000,
    pseudo: pseudo,
    anonymous: anonymous,
  ...
  Future<UserSession> _refresh(UserSession current) async {
    final data = await _request(
      Uri.parse('https://securetoken.googleapis.com/v1/token?key=$firebaseKey'),
      method: 'POST',
      headers: {'Content-Type': 'application/x-www-form-urlencoded'},
      body:
          'grant_type=refresh_token&refresh_token=${Uri.encodeQueryComponent(current.refreshToken)}',
    );
    return _persist(
      UserSession(
        ...
        expiresAt:
            DateTime.now().millisecondsSinceEpoch +
            ((num.tryParse('${data['expires_in'] ?? 3600}') ?? 3600).toInt() *
                1000) -
            60000,
  ...
  String _email(String pseudo) =>
      '${pseudo.toLowerCase().replaceAll(RegExp(r'[^a-z0-9_.-]'), '')}@fasobiblio.local';
  ...
  Future<UserSession> signup(
    ...
    await authenticated(
      '/api/register-recovery',
      method: 'POST',
      body: {'pseudo': pseudo, 'phone': phone, 'phoneCountry': phoneCountry},
    );
    return next;
  }
  ...
  Future<String> recoverAccount(
    ...
    final data = await _request(
      Uri.parse('$api/api/recover-account'),
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        'pseudo': pseudo.trim(),
  ...
  Future<void> resetPassword(String code, String password) async {
    await _request(
      Uri.parse(
        'https://identitytoolkit.googleapis.com/v1/accounts:resetPassword?key=$firebaseKey',
      ),
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({'oobCode': code, 'newPassword': password}),
    );
  }
  ...
  Future<String> transcribeAudio(List<int> audio) async {
    ...
    final result = await _request(
      Uri.parse('$api/api/mobile/transcribe'),
      method: 'POST',
      headers: {
    ...
    final text = result is Map ? '${result['text'] ?? ''}'.trim() : '';
    if (text.isEmpty) throw Exception('Aucune parole reconnue.');
    return text;
  }

  Future<List<Book>> catalog() async {
    final data = await _request(
      Uri.parse('$database/documents.json'),
  ...
  Future<List<AppNotification>> notifications() async {
    final data = await _request(
      Uri.parse('$database/notifications.json'),
    ...
    if (data is Map) {
      values.addAll(
        data.entries
            ...
            .map(
              (entry) => AppNotification.fromJson(
                '${entry.key}',
                Map<String, dynamic>.from(entry.value),
              ),
            ),
    ...
    try {
      ...
      if (items is Map) {
        values.addAll(
          items.entries
              .where((entry) => entry.value is Map)
              .map(
                (entry) => AppNotification.fromJson('${entry.key}', {
                  ...Map<String, dynamic>.from(entry.value),
                  'uid': _session?.uid ?? '',
                }),
              ),
      ...
      /* General announcements remain available on older servers. */
    }
  ...
  Future<Map<String, dynamic>> setting(String path) async {
    final encodedPath = path
        ...
        .join('/');
    final data = await _request(
      Uri.parse('$database/$encodedPath.json'),
      timeout: const Duration(seconds: 12),
    );
  ...
  Future<void> markNotificationRead(String notificationId) async {
    ...
    await _request(
      Uri.parse(
        '$database/notif_reads/${Uri.encodeComponent(auth.uid)}/${Uri.encodeComponent(notificationId)}.json?auth=${Uri.encodeQueryComponent(auth.idToken)}',
      ),
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: 'true',
  ...
  Future<void> markAllNotificationsRead(Iterable<String> ids) async {
    ...
    await _request(
      Uri.parse(
        '$database/notif_reads/${Uri.encodeComponent(auth.uid)}.json?auth=${Uri.encodeQueryComponent(auth.idToken)}',
      ),
      method: 'PATCH',
      headers: {'Content-Type': 'application/json'},
  ...
  Future<List<DocumentReview>> reviews(String documentId) async {
    final data = await _request(
      Uri.parse(
        '$database/document_reviews/${Uri.encodeComponent(documentId)}.json',
      ),
      timeout: const Duration(seconds: 12),
    );
    if (data is! Map) return [];
  ...
  Future<void> sendSupportMessage({
    ...
    await authenticated(
      ...
      method: 'POST',
      body: {
        'nom': auth.anonymous ? 'Lecteur invité' : auth.pseudo,
        'email': email.trim(),
  ...
  Future<void> sendSuggestion({
    ...
    await _request(
      Uri.parse(
        '$database/suggestions.json?auth=${Uri.encodeQueryComponent(auth.idToken)}',
      ),
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        'name': auth.anonymous ? 'Lecteur invité' : auth.pseudo,
        'title': title.trim(),
  ...
  Future<Map<String, String>> documentFile(String id, String mode) async {
    final data = await authenticated(
      '/api/doc-file?docId=${Uri.encodeQueryComponent(id)}&mode=$mode',
    );
    return {
      'url': '${data['fileUrl'] ?? data['file_url'] ?? ''}',
      'name': '${data['fileName'] ?? data['file_name'] ?? 'Document.pdf'}',
    };
  }
  ...
  Future<Map<String, String>> startDocumentPayment({
  ...
  Future<Map<String, String>> startSubscriptionPayment({
  ...
  Future<Map<String, String>> startDonationPayment({
    required int amount,
    required String phone,
    required String pseudo,
  }) async {
  ...
  Future<bool> checkAccess(String docId) async {
    final data = await authenticated(
      '/api/check-access?docId=${Uri.encodeQueryComponent(docId)}',
    );
  ...
  Future<Map<String, dynamic>?> mySubscription() async {
    ...
    return data is Map && data['subscription'] is Map
        ? Map<String, dynamic>.from(data['subscription'])
        : null;
  }

  Future<String> assistant(
    String question, {
    ...
    final data = await _request(
      Uri.parse('$api/api/chat'),
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        'systemPrompt': 'Tu es l’assistant pédagogique de Fasobiblio. Réponds clairement en français. Utilise du Markdown propre pour structurer la réponse : titres, listes, gras, italique et liens si utile. Ne montre jamais les marqueurs Markdown sans raison.',
        'userPrompt': question,
