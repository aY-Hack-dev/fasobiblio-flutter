...
class AppState extends ChangeNotifier {
  ...
  Future<void> logout() async{await PushService.instance.detachAccount();session=await api.logout();notifications.removeWhere((item)=>item.uid.isNotEmpty);await store.saveJson(LocalStore.notificationsKey,notifications.map((item)=>item.toJson()).toList());subscription=null;purchased={};await store.clearAccountAccess();notifyListeners();}
