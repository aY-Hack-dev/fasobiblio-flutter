...
Future<void> _showRecoverySheet(BuildContext context, AppState state) async {
  ...
  await showModalBottomSheet<void>(
    ...
    builder: (sheetContext) => StatefulBuilder(builder: (context, setModalState) {
      Future<void> submit() async {
        final name = pseudo.text.trim();
        final digits = phone.text.replaceAll(RegExp(r'\D'), '');
        if (name.length < 3) { setModalState(() => error = 'Saisissez le pseudo du compte à récupérer.'); return; }
        if (!RegExp(r'^\d{8,10}$').hasMatch(digits)) { setModalState(() => error = 'Saisissez le numéro de récupération associé au compte.'); return; }
        setModalState(() { busy = true; error = null; });
        try {
          await state.api.sendSupportMessage(type: 'Récupération de compte', message: 'Demande de récupération de compte. Pseudo : $name. Téléphone déclaré : $digits.');
          if (!sheetContext.mounted) return;
          Navigator.pop(sheetContext);
          if (context.mounted) showToast(context, 'Demande envoyée. Fasobiblio vérifiera les informations du compte.', success: true);
        } catch (e) {
          if (sheetContext.mounted) setModalState(() { busy = false; error = friendlyFailure(e, action: 'envoyer la demande de récupération'); });
        }
      }
      final dark = Theme.of(context).brightness == Brightness.dark;
      return SafeArea(top: false, child: Material(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Row(children: [
              Container(width: 46, height: 46, decoration: BoxDecoration(color: AppColors.sky, borderRadius: BorderRadius.circular(15)), child: const Icon(AppIcons.lock, color: AppColors.blue)),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Récupérer mon compte', style: Theme.of(context).textTheme.headlineSmall), const SizedBox(height: 3), const Text('Indiquez le pseudo et le numéro utilisés lors de l’inscription.', style: TextStyle(fontSize: 11.5, color: AppColors.muted))])),
              IconButton(onPressed: busy ? null : () => Navigator.pop(sheetContext), icon: const Icon(AppIcons.close)),
            ]),
            const SizedBox(height: 20),
            TextField(controller: pseudo, enabled: !busy, textInputAction: TextInputAction.next, decoration: const InputDecoration(labelText: 'Nom d’utilisateur', prefixIcon: Icon(AppIcons.user))),
            const SizedBox(height: 12),
            TextField(controller: phone, enabled: !busy, keyboardType: TextInputType.phone, decoration: const InputDecoration(labelText: 'Téléphone de récupération', hintText: 'Ex : 70 12 34 56', prefixIcon: Icon(AppIcons.phone))),
            if (error != null) Padding(padding: const EdgeInsets.only(top: 12), child: Text(error!, style: const TextStyle(fontSize: 11.5, color: Color(0xFFB91C1C), fontWeight: FontWeight.w700))),
            const SizedBox(height: 16),
            SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: busy ? null : submit, icon: busy ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(AppIcons.send), label: Text(busy ? 'Envoi…' : 'Envoyer la demande'))),
          ]),
        ),
      ));
    }),
...
Future<void> showAuthSheet(BuildContext context, AppState state, {required bool signup}) async {
  ...
  await showModalBottomSheet<void>(context: context,isScrollControlled: true,useSafeArea: true,backgroundColor: Colors.transparent,barrierColor: Colors.black.withValues(alpha: .52),builder: (sheetContext) => StatefulBuilder(builder: (context, setModalState) {
    ...
    return SafeArea(top:false,child:Material(color:dark?const Color(0xFF111B2C):Colors.white,borderRadius:const BorderRadius.vertical(top:Radius.circular(30)),clipBehavior:Clip.antiAlias,child:SingleChildScrollView(padding:EdgeInsets.fromLTRB(22,10,22,MediaQuery.viewInsetsOf(context).bottom+24),child:Column(mainAxisSize:MainAxisSize.min,children:[
      ...
      if(!signup) Align(alignment: Alignment.centerRight, child: TextButton(onPressed: busy ? null : () async { Navigator.pop(sheetContext); if (context.mounted) await _showRecoverySheet(context, state); }, child: const Text('Mot de passe oublié ?'))),
