...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _open(AppNotification item) async {
    final opening = _expandedId != item.id;
    setState(() => _expandedId = opening ? item.id : null);
    if (opening && !state.notificationReads.contains(item.id)) {
      try {
      ...
      final items =
          state.notifications
              .where(
                (item) =>
                    !_unreadOnly ||
                    !state.notificationReads.contains(item.id) ||
                    item.id == _expandedId,
              )
              .toList()
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Flexible(
                    child: ChoiceChip(
                      label: Text('Non lues (${state.unreadNotifications})'),
                      selected: _unreadOnly,
              ...
              child: items.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final showHeading =
                            index == 0 ||
                            _period(items[index - 1].createdAt) != period;
                        return Column(
