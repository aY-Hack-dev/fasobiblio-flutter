...
class ProfileScreen extends StatelessWidget {
  ...
  void information(BuildContext context, InformationKind kind) =>
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => InformationScreen(kind: kind, state: state),
        ),
      );
      ...
      children: [
          ...
          children: [
            _RowItem(
              ...
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => ReadingActivityScreen(state: state),
                ),
              ),
            ),
            _RowItem(
              icon: Icons.edit_note,
              title: 'Mes notes et passages',
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) =>
                      ReadingActivityScreen(state: state, notes: true),
                ),
              ),
            ),
            _RowItem(
            ...
            _RowItem(
              ...
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => CollectionsScreen(state: state),
                ),
              ),
            ),
            _RowItem(
              icon: AppIcons.fileCheck,
              title: 'Documents hors connexion',
              onTap: () => Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => const OfflineDocumentsScreen(),
                ),
              ),
            ),
            _RowItem(
        ...
        _Group(
          children: [
            ...
            _RowItem(
              icon: AppIcons.heart,
              title: 'Faire un don',
              onTap: () => makeDonation(context, state),
            ),
          ],
        ),
        const SizedBox(height: 14),
        ...
        _Group(
          children: [
            _RowItem(
              icon: AppIcons.info,
              title: 'À propos de Fasobiblio',
              onTap: () => information(context, InformationKind.about),
            ),
            _RowItem(
              icon: AppIcons.fileText,
              title: 'Conditions d’utilisation',
              onTap: () => information(context, InformationKind.terms),
            ),
            _RowItem(
              icon: AppIcons.shield,
              title: 'Politique de confidentialité',
              onTap: () => information(context, InformationKind.privacy),
            ),
            _RowItem(
              icon: AppIcons.scale,
              title: 'Mentions légales et contact',
              onTap: () => information(context, InformationKind.legal),
            ),
          ],
        ),
            ...
            onPressed: () async {
              final confirmed = await showDialog<bool>(
                ...
                builder: (context) => AlertDialog(
                  ...
                  content: const Text(
                    'Vous pourrez vous reconnecter pour retrouver vos accès.',
                  ),
                  actions: [
                    TextButton(
              ...
              try {
                ...
                if (context.mounted) {
                  showToast(
                    context,
                    friendlyFailure(error, action: 'vous déconnecter'),
                  );
                }
              }
            },
      ...
      if (!connected)
        Row(
          children: [
            Expanded(
              child: FilledButton(
                onPressed: () => showAuthSheet(context, state, signup: true),
                child: const Text(
                  'Créer un compte',
                  textAlign: TextAlign.center,
                ),
            ...
            Expanded(
              child: FilledButton(
                onPressed: () => showAuthSheet(context, state, signup: false),
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      ...
      children: [
        const Row(
          children: [
            Icon(AppIcons.sun, size: 18),
            SizedBox(width: 8),
            Text('Apparence', style: TextStyle(fontWeight: FontWeight.w900)),
          ],
        ),
  ...
  Widget build(BuildContext context) => InkWell(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: selected
              ? AppColors.blueDeep.withValues(alpha: .35)
              : Colors.transparent,
        ),
      ),
      child: Column(
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              height: 5,
              decoration: BoxDecoration(
                color: dark ? const Color(0xFF43516A) : const Color(0xFFD4DDEA),
                borderRadius: BorderRadius.circular(10),
              ),
            ),
            Flexible(child: child),
            SizedBox(
              height: MediaQuery.viewPaddingOf(context).bottom == 0 ? 10 : 4,
            ),
          ],
        ),
      ),
...
class _SupportSheetState extends State<_SupportSheet> {
  ...
  Future<void> submit() async {
    ...
    try {
      await widget.state.api.sendSupportMessage(
        type: type,
      ...
      if (messengerContext.mounted) {
        showToast(
          messengerContext,
          'Votre message a bien été envoyé à Fasobiblio.',
          success: true,
        );
      }
    } catch (e) {
      if (mounted) {
        setState(() {
          busy = false;
          error = friendlyFailure(e, action: 'envoyer votre message');
        });
      }
    }
  }
  ...
  Widget build(BuildContext context) => SingleChildScrollView(
    ...
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 5),
        const Text(
          'Quel sujet pouvons-nous traiter ?',
          style: TextStyle(fontSize: 12, color: AppColors.muted),
        ),
        ...
        Wrap(
          ...
          children: topics.map((topic) {
            ...
            return ChoiceChip(
              selected: selected,
              onSelected: busy ? null : (_) => setState(() => type = topic.$1),
              showCheckmark: false,
              avatar: Icon(
                topic.$2,
                size: 18,
        ...
        SizedBox(
          ...
          child: FilledButton.icon(
            onPressed: busy || controller.text.trim().length < 4
                ? null
                : submit,
            icon: busy
                ? const SizedBox(
            ...
            label: Text(busy ? 'Envoi en cours…' : 'Envoyer'),
          ),
        ),
      ],
    ),
...
class _SuggestionSheetState extends State<_SuggestionSheet> {
  ...
  Future<void> submit() async {
    ...
    try {
      await widget.state.api.sendSuggestion(
      ...
      if (messengerContext.mounted) {
        showToast(
          messengerContext,
          'Suggestion envoyée. Merci de contribuer au catalogue !',
          success: true,
        );
      }
    } catch (e) {
      if (mounted) {
        setState(() {
          busy = false;
          error = friendlyFailure(e, action: 'envoyer votre suggestion');
        });
      }
    }
  }
    ...
    child: Column(
      ...
      children: [
        ...
        TextField(
          controller: subject,
          enabled: !busy,
          decoration: const InputDecoration(labelText: 'Matière ou thème'),
        ),
        const SizedBox(height: 10),
        TextField(
          controller: level,
        ...
        TextField(
          ...
          minLines: 3,
          maxLines: 5,
          decoration: const InputDecoration(labelText: 'Précisions'),
        ),
        if (error != null)
          Padding(
        ...
        SizedBox(
          width: double.infinity,
          child: FilledButton.icon(
            onPressed: busy || title.text.trim().length < 2 ? null : submit,
            icon: busy
                ? const SizedBox(
                    width: 17,
                    height: 17,
                ...
                : const Icon(AppIcons.send),
            label: Text(busy ? 'Envoi en cours…' : 'Envoyer la suggestion'),
          ),
        ),
      ],
    ),
