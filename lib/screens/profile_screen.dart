...
class ProfileScreen extends StatelessWidget {
  ...
  Future<void> _support(BuildContext context) async {
    ...
    final result = await showModalBottomSheet<(String, String)>(context: context, isScrollControlled: true, useSafeArea: true, builder: (_) => const _SupportSheet());
  ...
  Future<void> _suggest(BuildContext context) async {
    if (!requireInternet(context, state)) return;
    final result = await showModalBottomSheet<(String, String, String, String)>(context: context, isScrollControlled: true, useSafeArea: true, builder: (_) => const _SuggestionSheet());
    if (result == null || !context.mounted) return;
    try {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(children: [
      ...
      _Menu(icon: Icons.local_library_outlined, title: 'Ma bibliothèque', subtitle: 'Favoris, liste de lecture et achats', onTap: onLibrary),
      _Menu(icon: Icons.download_done_rounded, title: 'Documents hors connexion', subtitle: 'Lectures conservées et dossier Download/Fasobiblio', onTap: () => information(context, InformationKind.downloads)),
      _Menu(icon: Icons.notifications_none_rounded, title: 'Notifications', subtitle: state.unreadNotifications == 0 ? 'Vous êtes à jour' : '${state.unreadNotifications} nouvelle(s) annonce(s)', onTap: onNotifications, badge: state.unreadNotifications),
      const SectionTitle('Services Fasobiblio'),
      _Menu(icon: Icons.auto_awesome_rounded, title: 'Assistant d’étude', subtitle: 'Posez vos questions à l’assistant Fasobiblio', onTap: onAssistant),
      _Menu(icon: Icons.lightbulb_outline_rounded, title: 'Suggérer un document', subtitle: 'Proposez un livre, un cours ou une ressource', onTap: () => _suggest(context)),
      _Menu(icon: Icons.support_agent_rounded, title: 'Aide et assistance', subtitle: 'Écrivez directement à l’équipe Fasobiblio', onTap: () => _support(context)),
      const SectionTitle('Accessibilité'),
      _DarkModeTile(state: state),
      const SectionTitle('À propos'),
      _Menu(icon: Icons.info_outline_rounded, title: 'À propos de Fasobiblio', subtitle: 'Notre mission et le fonctionnement de l’application', onTap: () => information(context, InformationKind.about)),
...
class _ProfileCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(children: [
      ...
      if (connected)
        OutlinedButton.icon(onPressed: state.logout, icon: const Icon(Icons.logout_rounded), label: const Text('Se déconnecter'), style: OutlinedButton.styleFrom(foregroundColor: Colors.white, side: const BorderSide(color: Color(0x66FFFFFF))))
      else
