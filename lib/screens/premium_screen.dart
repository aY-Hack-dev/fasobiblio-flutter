            ...
            child: SizedBox(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, i) {
                  ...
                  final name = nameValue is Map
                      ? '${nameValue['fr'] ?? (nameValue.values.isNotEmpty ? nameValue.values.first : 'Offre')}'
                      : '${nameValue ?? offer['id'] ?? 'Offre'}';
                  ...
                  return _OfferCard(
                    ...
                    durationDays: int.tryParse('${offer['durationDays'] ?? 30}') ?? 30,
                    featured: i == 0,
        ...
        SliverToBoxAdapter(
          child: books.isEmpty
              ...
              : BookGrid(
                  ...
                  onBook: onBook,
                  onFavorite: (book) => state.toggleFavorite(book.id),
                ),
        ),
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          gradient: const LinearGradient(
            colors: [Color(0xFF0A1D3D), Color(0xFF123F8C), AppColors.blue],
          ...
          children: [
            Positioned(
              ...
              child: Container(
                width: 170,
                height: 170,
                decoration: BoxDecoration(color: Colors.white.withValues(alpha: .06), shape: BoxShape.circle),
              ),
            ),
            Positioned(
              right: 8,
              bottom: -34,
              child: Icon(AppIcons.premium, size: 130, color: const Color(0xFFFFD166).withValues(alpha: .10)),
            ),
            Column(
              ...
              children: [
                Row(
                  children: [
                    Container(
                      ...
                      height: 50,
                      decoration: BoxDecoration(
                        color: const Color(0xFFFFD166).withValues(alpha: .15),
                        borderRadius: BorderRadius.circular(16),
                        border: Border.all(color: const Color(0xFFFFD166).withValues(alpha: .35)),
                      ),
                      child: const Icon(AppIcons.premium, color: Color(0xFFFFD166), size: 27),
                    ),
                    const Spacer(),
                    Container(
                      ...
                      child: Text(
                        active ? 'PREMIUM ACTIF' : '$bookCount TITRES',
                        style: const TextStyle(fontSize: 9, letterSpacing: .8, fontWeight: FontWeight.w900, color: Colors.white),
                      ),
                    ),
                  ],
                ),
                ...
                const Text(
                  'Accédez aux ressources avancées et construisez une bibliothèque qui vous accompagne vraiment dans vos études.',
                  style: TextStyle(fontSize: 12, height: 1.5, color: Color(0xFFDCE7FF)),
                ),
                ...
                const Wrap(
                  ...
                  runSpacing: 8,
                  children: [
                    _BenefitChip(icon: AppIcons.bookOpen, label: 'Ressources premium'),
                    _BenefitChip(icon: AppIcons.download, label: 'Lecture hors ligne'),
                    _BenefitChip(icon: AppIcons.sparkles, label: 'Expérience complète'),
                  ],
                ),
              ],
            ),
...
class _BenefitChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 14, color: const Color(0xFFFFD166)), const SizedBox(width: 6), Text(label, style: const TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: Colors.white))]),
      );
}
          ...
          children: [
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('${subscription['planName'] ?? 'Premium actif'}', style: const TextStyle(fontWeight: FontWeight.w900, color: AppColors.ink)),
                const SizedBox(height: 3),
                Text(subscription['lifetime'] == true ? 'Votre accès Premium est valable à vie.' : 'Votre abonnement Premium est actif.', style: const TextStyle(fontSize: 11, color: AppColors.muted)),
              ]),
            ),
          ],
        ),
  ...
  Widget build(BuildContext context) => Material(
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              border: Border.all(color: featured ? const Color(0x55FFD166) : Theme.of(context).dividerColor.withValues(alpha: .55)),
              borderRadius: BorderRadius.circular(21),
              boxShadow: featured ? const [BoxShadow(color: Color(0x21102C5C), blurRadius: 18, offset: Offset(0, 10))] : null,
            ),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text(name, maxLines: 1, overflow: TextOverflow.ellipsis, style: AppTypography.display(size: 15, weight: FontWeight.w900, color: featured ? Colors.white : AppColors.ink))),
                if (featured) const Icon(AppIcons.premium, size: 18, color: Color(0xFFFFD166)),
              ]),
              const Spacer(),
