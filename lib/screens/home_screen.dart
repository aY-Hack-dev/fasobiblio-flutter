...
class HomeScreen extends StatelessWidget {
  ...
  final VoidCallback onExplore, onAssistant, onNotifications;
  final ValueChanged<Book> onBook;
  List<Book> get homeBooks =>
      state.books.where((b) => b.image.trim().isNotEmpty).toList();
  Book? get dailyBook {
    if (homeBooks.isEmpty) return null;
    final now = DateTime.now();
    final day =
        DateTime(now.year, now.month, now.day).millisecondsSinceEpoch ~/
        Duration.millisecondsPerDay;
    return homeBooks[day % homeBooks.length];
  }

  @override
  Widget build(BuildContext context) {
    final popular = [...homeBooks]..sort((a, b) => b.views.compareTo(a.views));
    final recent = [...homeBooks]
      ..sort((a, b) => b.createdAt.compareTo(a.createdAt));
    final premium = homeBooks.where((b) => b.isPremium).toList();
    ...
    return CustomScrollView(
      slivers: [
        SliverToBoxAdapter(
          child: _WelcomeBlock(count: state.books.length, onExplore: onExplore),
        ),
        if (state.catalogPending) ...[
          const SliverToBoxAdapter(
            child: SectionTitle('Populaires maintenant'),
          ),
          SliverToBoxAdapter(child: DocumentSkeleton(state: state)),
          const SliverToBoxAdapter(child: SectionTitle('Nouveautés')),
          SliverToBoxAdapter(child: DocumentSkeleton(state: state)),
        ],
        if (featured != null)
          SliverToBoxAdapter(
            child: _Featured(book: featured, onTap: () => onBook(featured)),
          ),
        if (categories.isNotEmpty) ...[
          const SliverToBoxAdapter(child: SectionTitle('Explorer les rayons')),
          SliverToBoxAdapter(
            child: _Categories(values: categories, onExplore: onExplore),
          ),
        ],
        ...
        SliverToBoxAdapter(child: _StudyCard(onTap: onAssistant)),
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    child: ListView.separated(
      ...
      itemBuilder: (_, i) => ActionChip(
        ...
        label: Text(
          categoryLabel(values[i]),
          maxLines: 1,
          overflow: TextOverflow.ellipsis,
        ),
        side: BorderSide(color: AppColors.blue.withValues(alpha: .16)),
...
class _Shelf extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    children: [
      ...
      SizedBox(
        height:
            ((MediaQuery.sizeOf(context).width * .50).clamp(180.0, 230.0) -
                    18) /
                .72 +
            66 +
        ...
        child: const Row(
          children: [
            CircleAvatar(
              radius: 25,
              backgroundColor: Colors.white,
              child: Icon(AppIcons.brain, color: AppColors.blue),
            ),
            SizedBox(width: 14),
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(fontSize: 12, color: Color(0xFFDDE8FF)),
                  ),
                ],
              ),
            ),
