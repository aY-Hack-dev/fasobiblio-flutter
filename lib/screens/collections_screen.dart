...
class _CollectionsScreenState extends State<CollectionsScreen> {
  ...
  late final String storageKey =
      'collections.${widget.state.assistantAccountKey}';
  @override
  void initState() {
    super.initState();
    load();
  ...
  Future<void> save() async {
    try {
      await widget.state.store.saveJson(storageKey, collections);
    } catch (e) {
      if (mounted) {
        showToast(context, 'La collection n’a pas pu être enregistrée.');
      }
    }
  }
  ...
  Future<void> create() async {
    ...
    final name = await showDialog<String>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          maxLength: 50,
          decoration: const InputDecoration(hintText: 'Ex. Mes cours'),
        ),
        actions: [
          TextButton(
    ...
    if (collections.containsKey(name)) {
      showToast(context, 'Cette collection existe déjà.');
      return;
    }
  ...
  Future<void> rename(String oldName) async {
    ...
    final name = await showDialog<String>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(controller: controller, maxLength: 50),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Annuler'),
    ...
    if (collections.keys.any(
      (key) => normalizeSearch(key) == normalizeSearch(name),
    )) {
      showToast(context, 'Cette collection existe déjà.');
      return;
    }
    final documents = collections[oldName]!;
    setState(() {
  ...
  Future<void> edit(String name) async {
    ...
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, update) {
          final books = query.trim().isEmpty
              ? widget.state.books
              : searchCatalog(
                  widget.state.books,
                  query,
                ).map((hit) => hit.book).toList();
          return SizedBox(
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) => CheckboxListTile(
                      ...
                      onChanged: (v) => update(() {
                        v == true
                            ? selected.add(books[i].id)
                            : selected.remove(books[i].id);
                      }),
                    ),
                  ),
                ),
    ...
    if (result == true && mounted) {
      setState(() => collections[name] = selected.toList());
      await save();
    }
  }
    ...
    body: loading
        ...
        ? Center(
            child: FilledButton.icon(
              ...
              label: const Text('Créer ma première collection'),
            ),
          )
        : ListView(
                  ...
                  (entry) => ExpansionTile(
                    ...
                    children: [
                      Wrap(
                        children: [
                          TextButton(
                            onPressed: () => rename(entry.key),
                            child: const Text('Renommer'),
                          ),
                          TextButton.icon(
                            onPressed: () => edit(entry.key),
                            icon: const Icon(Icons.edit_outlined),
                            label: const Text('Choisir les documents'),
                          ),
                          TextButton(
                            onPressed: () async {
                              ...
                              if (yes == true && mounted) {
                                setState(() => collections.remove(entry.key));
                                await save();
                              }
                            },
                            child: const Text('Supprimer'),
                      ...
                      ...widget.state.books
                          .where((b) => entry.value.contains(b.id))
                          .map(
                            (b) => ListTile(
                              title: Text(
                                b.title,
