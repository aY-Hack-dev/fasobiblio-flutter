...
class _LibraryScreenState extends State<LibraryScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return CustomScrollView(slivers: [
      SliverToBoxAdapter(child: Align(alignment: Alignment.centerRight, child: TextButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => CollectionsScreen(state: widget.state))), icon: const Icon(Icons.folder_outlined), label: const Text('Mes collections')))),
      SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(18,18,18,4), child: Row(children:[Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('Ma bibliothèque',style:AppTypography.display(size:22,weight:FontWeight.w900)),const SizedBox(height:4),const Text('Retrouvez tout ce que vous avez gardé.',style:TextStyle(fontSize:12,color:AppColors.muted))])),Container(padding:const EdgeInsets.symmetric(horizontal:10,vertical:7),decoration:BoxDecoration(color:AppColors.sky,borderRadius:BorderRadius.circular(12)),child:Text('$total docs',style:const TextStyle(fontSize:12,fontWeight:FontWeight.w900,color:AppColors.blue)))]))),
      ...
      SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(18,18,18,8),child: Row(children:[Expanded(child:Text(section==0?'Mes favoris':section==1?'À lire plus tard':'Mes achats',style:Theme.of(context).textTheme.titleLarge)),Text('${books.length}',style:const TextStyle(fontSize:12,fontWeight:FontWeight.w900,color:AppColors.blueDeep))]))),
