...
class StartupScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (state.loading && state.books.isEmpty) {
      return const _LibraryLoadingScreen();
    }
    if (state.offline && state.books.isEmpty) {
      return const _OfflineCatalogSkeleton();
    }
    return AppShell(state: state);
  }
}

class _OfflineCatalogSkeleton extends StatefulWidget {
  const _OfflineCatalogSkeleton();

  @override
  State<_OfflineCatalogSkeleton> createState() => _OfflineCatalogSkeletonState();
}

class _OfflineCatalogSkeletonState extends State<_OfflineCatalogSkeleton> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        showToast(
          context,
          'Pas de connexion. Connectez-vous à Internet pour charger le catalogue Fasobiblio.',
        );
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final shade = Theme.of(context).dividerColor.withValues(alpha: .18);

    Widget bone({double? width, double height = 16}) => Container(
          width: width,
          height: height,
          decoration: BoxDecoration(
            color: shade,
                        ...
                        children: List.generate(
                          ...
                          (i) => Expanded(
                            child: Container(
                              ...
                              child: Column(
                                children: [
                                  Expanded(child: bone(height: 170)),
...
class _LibraryLoadingScreenState extends State<_LibraryLoadingScreen>
