...
class BookCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    child: Material(
      ...
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: BorderSide(color: Theme.of(context).dividerColor.withValues(alpha: .75))),
      clipBehavior: Clip.antiAlias,
      child: InkWell(
        onTap: onTap,
        child: Column(children: [
          Container(height: 3, decoration: const BoxDecoration(gradient: LinearGradient(colors: [AppColors.blue, AppColors.ink]))),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [const Icon(AppIcons.star, size: 14, color: Color(0xFFF4B740)), const SizedBox(width: 3), Text(book.rating > 0 ? book.rating.toStringAsFixed(1) : 'Nouveau', style: const TextStyle(fontSize: 10, color: AppColors.muted)), const Spacer(), if (book.isPremium) Text(book.price > 0 ? '${book.price.toInt()} F' : 'Premium', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.gold))]),
          ]),
        )),
        ]),
      ),
...
class BookGrid extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => LayoutBuilder(builder: (context, constraints) {
    ...
    final extent = ((cardWidth - 18) / .7 + 108).clamp(286.0, 326.0).toDouble();
    return GridView.builder(
      ...
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: columns, mainAxisExtent: extent, crossAxisSpacing: gap, mainAxisSpacing: gap),
