...
class _BookGridState extends State<BookGrid> {
 ...
 @override Widget build(BuildContext context)=>LayoutBuilder(builder:(context,constraints){final columns=constraints.maxWidth>=650?4:constraints.maxWidth>=470?3:2;const gap=14.0;final cardWidth=(constraints.maxWidth-32-gap*(columns-1))/columns;final extent=(cardWidth-18)/.68+110+MediaQuery.textScalerOf(context).scale(28);return Column(children:[GridView.builder(shrinkWrap:true,physics:const NeverScrollableScrollPhysics(),padding:const EdgeInsets.symmetric(horizontal:16,vertical:6),gridDelegate:SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:columns,mainAxisExtent:extent,crossAxisSpacing:gap,mainAxisSpacing:16),itemCount:books.length<visible?books.length:visible,itemBuilder:(_,i)=>BookCard(book:books[i],favorite:favorites.contains(books[i].id),onTap:()=>onBook(books[i]),onFavorite:onFavorite==null?null:()=>onFavorite!(books[i]),width:double.infinity)),if(books.length>visible)TextButton(onPressed:()=>setState(()=>visible+=24),child:const Text('Afficher plus'))]);});
}
